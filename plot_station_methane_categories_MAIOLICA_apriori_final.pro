...
;  FOR i=0,m-30 DO BEGIN ;TEST
 ...
 ENDIF ELSE IF Long(cyyyymm) eq (Long(sim.syyyymm)+2L) THEN BEGIN
    ...
    IF keyword_set(flask) THEN BEGIN      
      ...
      ELSE modfile = '/nas/arf/INVERSION/m_allweekly_flask_'+sn+'_'+sim.name+'_'+cyyyymm+'.dat'       
    ENDIF ELSE BEGIN
      IF keyword_set(nobg) THEN $
    ...
    IF keyword_set(flask) THEN BEGIN
      ...
      ELSE catfile = '/nas/arf/INVERSION/m_weekly_apost_categories_flask_'+sn+'_'+sim.name+'_'+qunc+'_'+cyyyymm+'.dat'
    ENDIF ELSE BEGIN
      IF keyword_set(nobg) THEN $
...
NCDF_CLOSE, ncid4
  ...
  IF keyword_set(flask) THEN BEGIN
    ...
    ELSE file='/home/arf/pers/IDL/EPS/URMEL/INVERSION/ch4_growthrates_flask_'+sn+'_'+sim.name+'_'+np+'_inversion_cat_'+qunc+'.eps'  
  ENDIF ELSE BEGIN
    IF keyword_set(nobg) THEN $
