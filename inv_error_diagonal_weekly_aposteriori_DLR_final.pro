...
;* MAIN PROGRAM
;******************************************************
PRO inv_error_diagonal_weekly_aposteriori_DLR_final,sim=sim,stats=stats,flask=flask,ufact=ufact,nobg=nobg,special=special,$
                                          startcf=startcf
...
n        = eyyyy*12L+emm-(syyyy*12L+smm)+1 ; number months in inversion ;FLO
  ...
  IF keyword_set(flask) THEN BEGIN
    testfile = sim.outdir+'inv_output_weekly_flask_DLR_prelim_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+$
...
;stop
  ...
  IF keyword_set(flask) THEN BEGIN
    filemean = '/nas/arf/INVERSION/FINAL/inv_errorcovariance_stations_allweekly_flask_DLR_'+sn+'_'+'mean_nov12.dat' 
  ENDIF ELSE BEGIN
  ...
  FOR ij=0,nyears DO BEGIN
    ...
    FOR im=0,nmonths-1 DO BEGIN
       ...
       IF keyword_set(flask) THEN BEGIN
         monfile = dirout+'inv_errorcovariance_wm_mismatchonly_aposteriori_flask_DLR_'+sn+'_'+sim.name+'_'+yyyymm+'.dat'        
       ENDIF ELSE BEGIN
