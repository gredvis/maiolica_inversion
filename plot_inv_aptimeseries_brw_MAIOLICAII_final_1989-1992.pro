...
;MAIN PROGRAM
;******************************************************************************
PRO plot_inv_aptimeseries_brw_MAIOLICAII_final_1989_1992,sim=sim,stn=stn,post=post
...
;  nyr  = 8
 ...
 nyr  = 4 ;flo
  ...
  ;  syear = STRCOMPRESS(string(2001+ij),/REM)
     ...
     syear = STRCOMPRESS(string(1989+ij),/REM)
...
;  nyears   = fix(STRMID(sim.eyyyymm,0,4))-2001+1
    ...
    nyears   = fix(STRMID(sim.eyyyymm,0,4))-1989+1 ;flo
;  syyyy    = 2001 & smm = 1
  syyyy    = 1990 & smm = 1 ;flo
  syyyy    = 1989 & smm = 2 ;flo
...
;  m        = eyyyy*12L+emm-(2000*12L+2)+1  
  m        = eyyyy*12L+emm-(1989*12L+2)+2  ;flo
  ...
  testfile = sim.outdir+'inv_output_weekly_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+sim.eyyyymm+'_'+qunc+'_nov12.txt'    
...
; NEW
  fcorr[*,*] = fhelp[0:m-1,*]
  FOR it=0,sim.ntrace-1 DO BEGIN
    sa[*,it] = sahelp[0:m-1,it]
    sp[*,it] = sphelp[0:m-1,it]
 ENDFOR
...
;  syra = '2001'
  ...
  yrange = [1750.,1980.]
  IF stn eq 'chr' THEN yrange = [1600.,1800.]
  IF stn eq 'gmi' THEN yrange = [1600.,1850.]
  IF stn eq 'cgo' THEN yrange = [1600.,1850.]
  IF stn eq 'cpt' THEN yrange = [1600.,1800.]
  IF stn eq 'smo' THEN yrange = [1600.,1850.]
  IF stn eq 'sey' THEN yrange = [1600.,1850.]
  IF stn eq 'nwr' THEN yrange = [1650.,1900.]
  IF stn eq 'alt ' THEN yrange = [1750.,1950.]
  IF stn eq 'hba' THEN yrange = [1600.,1800.]
  IF stn eq 'syo' THEN yrange = [1600.,1800.]
  IF stn eq 'ams' THEN yrange = [1600.,1800.]
  IF stn eq 'maa' THEN yrange = [1600.,1800.]
  IF stn eq 'mlo' THEN yrange = [1650.,1900.]
  IF stn eq 'asc' THEN yrange = [1600.,1800.]
  IF stn eq 'psa' THEN yrange = [1600.,1800.]
  IF stn eq 'nmb' THEN yrange = [1600.,1800.]
  ...
  IF keyword_set(post) THEN oplot,datall,zmpstat_plot[0:ntog-1],color=13,thick=3,noclip=0
