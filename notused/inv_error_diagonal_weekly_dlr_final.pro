...
;* MAIN PROGRAM
;******************************************************
PRO inv_error_diagonal_weekly_dlr_final,sim=sim,stats=stats,flask=flask,ufact=ufact,nobg=nobg,special=special
  ...
  IF keyword_set(flask) THEN BEGIN
    filemean = '/nas/arf/INVERSION/FINAL/inv_errorcovariance_stations_flask_DLR_'+sim.name+'_'+sn+'_'+'mean.dat'  
    ...
    FOR im=0,nmonths-1 DO BEGIN
      ...
      read_data_single_final,sim=sim,yyyymm=yyyymm,ch4obs=ch4,dtgobs=dtgobs,$
       ...
       IF keyword_set(flask) THEN BEGIN
         ...
         ELSE $
         monfile = '/nas/arf/INVERSION/FINAL/'+$
         'inv_errorcovariance_stations_wm_mismatchonly_flask_DLR_'+sn+'_'+sim.name+'_'+syear[ij]+mon[im]+'.dat'       
       ENDIF ELSE BEGIN
         monfile = '/nas/arf/INVERSION/FINAL/'+$
