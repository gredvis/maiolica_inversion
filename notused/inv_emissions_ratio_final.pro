...
;*************************************************************
PRO inv_emissions_ratio_final,sim=sim,stats=stats,flask=flask,nobg=nobg,special=special
  ...
  IF keyword_set(flask) THEN BEGIN
    ...
    ELSE apostfile = sim.outdir+'inv_output_weekly_flask_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+$
                sim.eyyyymm+'_'+qunc+'_nov12.txt' 
  ENDIF ELSE BEGIN
...
; FOR l=0,sim.ntrace-1 DO BGpost[k] += mch4[33+l,k]*mseas[l]    
 ...
 FOR l=0,sim.ntrace-1 DO BGpost[k,l] += mch4[192+l,k]*mseas[l]    
    ...
    IF keyword_set(flask) THEN BEGIN
      ...
      ELSE outfile = '/nas/arf/INVERSION/FINAL/m_weekly_apost_categories_flask_'+sn+'_'+sim.name+'_'+qunc+'_'+cyyyymm+'.dat'
    ENDIF ELSE BEGIN
