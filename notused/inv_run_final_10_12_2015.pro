...
;-
PRO inv_run_final,sim=sim,Hdump=Hdump,weekly=weekly,keeppos=keeppos,$
            serdllh=serdllh,sumdllh=sumdllh,serzlen=serzlen,sumzlen=sumzlen,flask=flask,rapriori=rapriori,$
            stats=stats,startcf=startcf,nobg=nobg,special=special
...
;IF Long(cyyyymm) eq (Long(sim.syyyymm)+4L)  THEN stop
    ...
    ;;***************************************************
     ...
     ;;***************************************************  
     inv_determine_aposteriori_final,sim=sim,yyyymm=cyyyymm,fprev=fprev,fcorr=fcorr,$
                               ...
                               flask=flask,rapriori=rapriori,stats=stats,startcf=startcf,nobg=nobg,special=special
...
;stop
  ...
  ENDFOR
     IF i GE (nopt-1) THEN BEGIN
        ...
        ; and is now representative of month i-(nopt-1)
         ...
         FOR j=0,sim.ntrace-1 DO BEGIN
           ...
           Qp3[i-(nopt-2),0:sim.ntrace-1,j] = Qpout[sim.ntrace:sim.ntrace*2-1,sim.ntrace+j]
           Qp2[i-(nopt-3),0:sim.ntrace-1,j] = Qpout[2*sim.ntrace:sim.ntrace*3-1,2*sim.ntrace+j]
         ENDFOR         
     ...
     IF i EQ (n-1) THEN BEGIN
       ...
       SP2[i,*]        = Spsave[i-1,*]
       Qp2[i,*,*]      = Qpsave[i-1,*,*]
       SP3[i,*]        = Spsave[i-2,*] ;flo NEW
       Qp3[i,*,*]      = Qpsave[i-2,*,*] ;flo NEW
...
;stop
  ...
  IF keyword_set(weekly) THEN BEGIN
    IF keyword_set(rapriori) THEN BEGIN
      ...
      testfile = sim.outdir+'inv_output_weekly_flask_prelim_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+sim.eyyyymm+'_'+qunc+'_nov12.txt' 
...
;print,((exp(Spsave))*365*1e-9)/((Sasave)*365*1e-9)
  ...
  matrixfile = sim.outdir+'inv_matrix_weekly_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+sim.eyyyymm+'_'+qunc+'_nov12.txt'
...
xifile=sim.outdir+'xi_statstics_'+sn+'_'+sim.name+'_'+sim.syyyymm+'-'+$
