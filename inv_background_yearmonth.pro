...
;-
PRO inv_background_yearmonth,sim=sim,yyyymm=yyyymm,fprev=fprev,$
...
;        fcorr[it] = (mass4e[it]*fprev[it]+mass3e[it]*em3_apost[it]/sa[it])/mass4a[it]  ; normal version
    ...
    ; FOR it=0,sim.ntrace-1 DO BEGIN
     ;  fcorr[it] = em3_apost[it]/sa[it]

;        fcorr[it]=startcf[it] ; FLO TEST 23 NOV
...
;;; NEW just (mass5e+M4e)/M5a;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 FOR i=0,7 DO BEGIN

IF it eq HS[i] THEN fcorr[it]=((total(mass4e[HS[*]])/8)*total(fprev[HS[*]]/8)+total(mass3e[HS[*]]/8))/total(mass4a[HS[*]]/8)

ENDFOR

 FOR i=0,12 DO BEGIN

IF it eq Trop[i] THEN fcorr[it]=((total(mass4e[Trop[*]])/13)*total(fprev[Trop[*]]/13)+total(mass3e[Trop[*]]/13))/total(mass4a[Trop[*]]/13)
ENDFOR

 FOR i=0,26 DO BEGIN
IF it eq HN[i] THEN fcorr[it]=((total(mass4e[HN[*]])/27)*total(fprev[HN[*]]/27)+total(mass3e[HN[*]]/27))/total(mass4a[HN[*]]/27)
ENDFOR
