...
;MAIN PROGRAM
;******************************************************************************
PRO inv_determine_aposteriori_DLR_final,sim=sim,yyyymm=yyyymm,fprev=fprev,fcorr=fcorr,$
                              em3_apost=em3_apost,Spin=Spin,Qpin=Qpin,$
  ...
  IF keyword_set(weekly) THEN BEGIN
    ...
    free_lun,lun
  ENDIF ELSE BEGIN
    read_model_data_month_DLR_final,sim=sim,yyyymm=yyyymm,stats=statsc,dtg=dtgc,mch4=mch4,mall=mall
...
;stop
 ...
 ENDIF ELSE BEGIN
  ...
  IF keyword_set(Hdump) THEN BEGIN
    IF keyword_set(weekly) THEN BEGIN
      IF keyword_set(flask) THEN BEGIN
        outfile = sim.hdir+'inv_sensitivity_weekly_flask_DLR_'+sn+'_'+sim.name+'_'+yyyymm+'_nov12.txt'        
      ENDIF ELSE BEGIN
    ...
    printf,lun,H
    free_lun,lun
    outfile2 = sim.hdir+'inv_names_weekly_DLR_'+sn+'_'+sim.name+'_'+yyyymm+'_nov12.txt'
  ...
  IF keyword_set(weekly) THEN BEGIN
    IF keyword_set(rapriori) THEN BEGIN
      IF keyword_set(flask) THEN BEGIN
        ...
        ELSE errorfile = direrror+'inv_errorcovariance_stations_wm_mismatchonly_flask_DLR_'+sn+'_'+sim.name+'_'+yyyymm+'.dat'
      ENDIF ELSE BEGIN
    ...
    ENDIF ELSE BEGIN    
      IF keyword_set(flask) THEN BEGIN
        errorfile = direrror+'inv_errorcovariance_wm_mismatchonly_aposteriori_flask_DLR_'+sn+'_'+sim.name+'_'+yyyymm+'.dat'         
      ENDIF ELSE BEGIN
